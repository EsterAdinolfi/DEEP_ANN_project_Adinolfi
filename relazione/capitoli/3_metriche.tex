...
\begin{enumerate}
    ...
    \end{itemize}
    Ai fini della visualizzazione comparativa si adotta un metodo \textbf{a credito pro-quota}: per ogni domanda il punto viene ripartito equamente tra tutte le minacce che raggiungono il valore massimo (in caso di pareggio ciascuna riceve $1/n$, dove $n$ è il numero di vincitori a pari merito). Questo evita l'assegnazione arbitraria del punto intero a un unico vincitore nei casi di parità.
    ...
    \begin{itemize}
        \item \textbf{Degraded}: $\Delta \leq -0.20$ (la minaccia fa crollare le performance del modello);
