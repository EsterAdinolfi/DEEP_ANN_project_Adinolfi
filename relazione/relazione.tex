...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Threat}: alla domanda viene aggiunta la frase minacciosa.
    \end{itemize}
\end{enumerate}

\noindent \textbf{Output:} \texttt{risultati/operational.json} ($1507 \times 12 = 18{,}084$ trial totali).
...
\begin{itemize}
    \item \textbf{Weighted}: media pesata sui 3 trial per condizione (1 riga per domanda);
    \item \textbf{Raw}: dati grezzi di tutti i singoli trial (12 righe per domanda).
\end{itemize}
...
\medskip
\textit{Nota: il framework è progettato per essere esteso ad altri modelli (es. Pythia-410M, Pythia-1B, LLaMA, Mistral). Sarà sufficiente modificare la costante \texttt{MODEL\_NAME} in \texttt{experiments\_1.py} e rieseguire la pipeline.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.55\textwidth]{fig3b_validity_vs_logconsistency.png}
\caption{Scatter validity vs coerenza log-testo: la correlazione perfetta ($r=1.0$) indica che il modello, quando genera testo valido, sceglie sempre l'opzione a logit più alta.}
\label{fig:validity_vs_log}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Scaling}: estendere l'analisi a modelli più grandi della famiglia Pythia (410M, 1B, 2.8B, 6.9B, 12B) per valutare l'effetto della dimensione sul bias politico
    \item \textbf{Modelli alternativi}: testare LLaMA, Mistral, e altri modelli open-source con diverse strategie di pre-training
    \item \textbf{Debiasing}: sperimentare tecniche di mitigazione del bias (prompt engineering, fine-tuning su dataset bilanciati, ecc.)
    \item \textbf{Cross-cultural analysis}: replicare l'analisi su survey non-USA per verificare se il bias liberal sia specifico del contesto americano o generalizzabile
    \item \textbf{Temporal dynamics}: analizzare come il bias evolve durante il training (checkpoints intermedi di Pythia)
\end{enumerate}
